...
class SyntheticClient {
  ...
  connect() {
    return new Promise((done, fail) => {
      ...
      this.ws = ws;
      ws.on('open', () => {
        this.rawSend(encodeHello(this.writer, `lt${this.index}`, this.index & 0xff, 0, 0).copy());
        done();
      });
...
async function scenarioBandwidth() {
  ...
  try {
    for (let i = 0; i < players; i++) {
      const c = new SyntheticClient(i, host.port, 0, { maxPlayers });
      await c.connect();
      clients.push(c);
    ...
    const joinLedger = clients.map((c) => ({
      chunks: c.chunksIn, chunkBytes: c.chunkBytes,
