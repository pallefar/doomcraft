...
export class Simulation {
  ...
  private resolveHitscan(
    ...
    if (monster >= 0) {
      ...
      this.damageEntity(monster, shooter.id, dmg, def.id, mflags, dx, dy, dz, def.variantSlot);
      return;
    }
    if (!victim) return;
  ...
  private resolveMelee(
    ...
    for (let e = 0; e < this.entCapacity; e++) {
      ...
      if (d > 1e-4 && (cx * dx + cy * dy + cz * dz) / d < cosLimit) continue;
      this.damageEntity(
        e, attacker.id, damageAtDistanceOf(def, d), def.id, 0, dx, dy, dz, def.variantSlot,
      );
    }
  }
    ...
    for (let i = 0; i < MAX_PROJECTILES; i++) {
      ...
      if (hitPlayer || hitEntity >= 0 || hitWorld) {
        ...
        if (hitPlayer) {
          ...
          this.damageEntity(
            hitEntity, this.projOwner[i], this.projDamage[i], weapon, 0, ndx, ndy, ndz,
            this.projVariant[i],
          );
        }
    ...
    if (def.splashRadius > 0) {
      ...
      for (let e = 0; e < this.entCapacity; e++) {
        ...
        if (dmg > 0.5 && !fromMonster) {
          const inv = d > 1e-4 ? 1 / d : 0;
          this.damageEntity(
            e, ownerId, dmg, weapon, DMG_SPLASH, cx * inv, cy * inv, cz * inv, def.variantSlot,
          );
        }
      }
    }
