...
| `DOOMCRAFT_AUDIT_DAYS` | `400` | Retention for the admin action log at `<DOOMCRAFT_DATA>/audit/<date>.ndjson`. Rows whose `verb` starts with `player.` are moderation records and are **kept past this bound** — `docs/INFRASTRUCTURE.md`: a ban that deletes itself is an exploit — so an expired day file is rewritten with only those rows rather than unlinked. |
| `DOOMCRAFT_ADMIN_TOKEN` | *(unset — no admin surface)* | The bearer for `/api/admin/*` **and the existence of `GET /admin`**. Unset means the console route 404s and every admin API answers 404 rather than 401, so an unconfigured deployment advertises nothing. Both sides of the compare are sha256'd before `timingSafeEqual`, so there is no length oracle; 20 failures per address per minute earns a `429`. |
| `DOOMCRAFT_TRUST_PROXY` | `0` | Read the client address from `x-forwarded-for`. **Only behind a proxy you control.** It is also the admin rate limiter's key, so an unconditional read here would be a limiter an attacker turns off with a header. |
...
| `GET /api/status` | Every room, plus the directory's own counters. Private join codes are cut out of both `key` and `name`, and **the world `seed` is cut too** — it lets anybody generate the room's world offline and know the map before they join. The operator's copy, with the seed and more, is `GET /api/admin/status`. |
| `GET /admin` | The operator console: one self-contained HTML page served by this process, never in the player bundle. **404 when `DOOMCRAFT_ADMIN_TOKEN` is unset.** The route itself is not token-gated — the page ships no data and asks for the token, which it holds in `sessionStorage` for that tab only — and every `/api/admin/*` call it makes is. |
| `GET /api/admin/status` | **Admin bearer.** The public status, plus `SignalHub.stats()` and a `{n, total, p50, p99, max}` rollup of every live `ConnectionStats`. Both were computed every second and served nowhere. |
| `GET /api/admin/whoami` | **Admin bearer.** This host's version document, and the list of operator verbs the console **cannot** perform yet, each with the reason and the phase that builds it. |
| `GET /api/admin/flags` · `POST /api/admin/flags/plan` · `POST /api/admin/flags` | **Admin bearer.** Read, review, merge. See `docs/PATCHING.md` §5 for the required `actor`/`reason`, the `409`, and the rollout ladder. |
| `GET /api/admin/audit[?since=&limit=]` | **Admin bearer.** The admin action log, newest first, with the before/after state on every row. |
| `GET /api/admin/player?key=<deviceId>[&limit=]` | **Admin bearer.** One player, by **exact** device id — the store declares `readdir` and never calls it, so there is no list and no search. Profile, economy, journal page and the reconciliation, all keyed by an 8-character handle; the id you send never comes back out. |
